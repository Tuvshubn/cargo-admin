...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    catch (err: unknown) { setError(err instanceof Error ? err.message : 'Нэвтрэх боломжгүй'); }
    finally { setLoading(false); }
  };

  return (
    <Box sx={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'linear-gradient(135deg, #0D47A1 0%, #1976D2 50%, #42A5F5 100%)', p:2 }}>
          ...
          <Box component="form" onSubmit={handleLogin}>
            ...
            <TextField fullWidth label="Нууц үг" type={showPass?'text':'password'}
              ...
              InputProps={{
                ...
                endAdornment:<InputAdornment position="end"><IconButton onClick={()=>setShowPass(p=>!p)} edge="end">{showPass?<VisibilityOff/>:<Visibility/>}</IconButton></InputAdornment>,
              }}
            />
