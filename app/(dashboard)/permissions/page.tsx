...
const PERM_LABELS: Record<string,string> = {
  ...
  'reports.view':'Харах','permissions.manage':'Удирдах','deliveries.manage':'Удирдах',
};
...
export default function PermissionsPage() {
  ...
  const fetch_ = async () => {
    ...
    data.forEach((p: {role:string;permission:string}) => { if(map[p.role]) map[p.role].push(p.permission); });
    setPerms(map);
    setLoading(false);
  };
  useEffect(()=>{ fetch_(); },[]);
  ...
  const toggle = async (role: string, perm: string) => {
    ...
    try {
      ...
      setPerms(prev=>({ ...prev, [role]: has ? prev[role].filter(p=>p!==perm) : [...(prev[role]||[]), perm] }));
                    ...
                    <Stack spacing={0.5} sx={{ mt:0.5 }}>
                      {items.map(perm=>(
                        <Stack key={perm} direction="row" justifyContent="space-between" alignItems="center" sx={{ py:0.5, px:1, borderRadius:1, '&:hover':{ bgcolor:'action.hover' } }}>
                          ...
                          <Switch size="small" checked={perms[role]?.includes(perm)||false} onChange={()=>toggle(role,perm)} disabled={saving}/>
                        </Stack>
                      ))}
                    </Stack>
                    <Divider sx={{ mt:1 }}/>
