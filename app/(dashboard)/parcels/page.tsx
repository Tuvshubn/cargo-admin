'use client';
import {
  Box, Card, CardContent, Button, Typography, Stack, Chip, Dialog, DialogTitle,
  DialogContent, DialogActions, TextField, MenuItem, Grid, Alert, IconButton,
  Tooltip, CircularProgress
} from '@mui/material';
import { DataGrid, GridColDef, GridToolbar } from '@mui/x-data-grid';
import { Add, Edit, Delete, FileDownload, Visibility, TrackChanges } from '@mui/icons-material';
...
const emptyForm = {
  ...
  cargo_type:'express', weight:'', quantity:'1', description:'',
...
export default function ParcelsPage() {
  ...
  const fetchParcels = useCallback(async () => {
    ...
    try {
      const { data } = await api.get('/parcels', {
        params: { search, status: statusFilter, page: paginationModel.page+1, limit: paginationModel.pageSize }
      });
  ...
  const save = async () => {
    try {
      ...
      setDialog(false); setForm(emptyForm); setEditId(null);
      fetchParcels();
  ...
  const columns: GridColDef[] = [
    { field: 'tracking_code', headerName: 'Трекинг код', width: 140, renderCell: p => (
    ...
    { field: 'cargo_type', headerName: 'Төрөл', width: 130, renderCell: p => (
      <Chip size="small" label={CARGO_TYPE_LABELS[p.value] || p.value} />
    )},
    { field: 'quantity', headerName: 'Тоо', width: 60 },
    { field: 'status', headerName: 'Статус', width: 150, renderCell: p => (
      <Chip size="small" label={STATUS_LABELS[p.value]} sx={{ bgcolor: STATUS_COLORS[p.value]+'22', color: STATUS_COLORS[p.value], fontWeight: 700, fontSize: 11 }} />
    )},
    { field: 'remaining_fee', headerName: 'Үлдэгдэл', width: 110, renderCell: p => (
      <Typography variant="body2" fontWeight={700} color={p.value > 0 ? 'error.main' : 'success.main'}>
        {parseInt(p.value||0).toLocaleString()}₮
      </Typography>
    )},
    { field: 'is_paid', headerName: 'Төлбөр', width: 90, renderCell: p => (
      <Chip size="small" label={p.value ? 'Төлсөн' : 'Хүлээгдэж'} color={p.value ? 'success' : 'warning'} />
    )},
    ...
    { field: 'actions', headerName: '', width: 130, sortable: false, renderCell: p => (
      <Stack direction="row" spacing={0.5}>
        <Tooltip title="Статус өөрчлөх">
          <IconButton size="small" color="primary" onClick={() => setStatusDialog({ id: p.row.id, status: p.row.status })}>
            <TrackChanges fontSize="small" />
          </IconButton>
        </Tooltip>
        <Tooltip title="Засах">
          <IconButton size="small" onClick={() => { setForm({ ...emptyForm, ...p.row }); setEditId(p.row.id); setDialog(true); }}>
            <Edit fontSize="small" />
          </IconButton>
        </Tooltip>
        {user?.role === 'admin' && (
    ...
    <Box>
      <Stack direction={{ xs:'column', sm:'row' }} justifyContent="space-between" alignItems={{ sm:'center' }} mb={3} spacing={2}>
        ...
        <Stack direction="row" spacing={1}>
          {user?.role === 'admin' && (
            <Button variant="outlined" startIcon={exporting ? <CircularProgress size={16} /> : <FileDownload />}
      ...
      <Dialog open={dialog} onClose={() => setDialog(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent dividers>
          <Grid container spacing={2} sx={{ pt: 1 }}>
            ...
            {[['kr_name','Нэр'],['kr_phone','Утасны дугаар'],['kr_address','Хаяг']].map(([k,l]) => (
              <Grid item xs={12} sm={4} key={k}>
                <TextField fullWidth size="small" label={l} value={(form as any)[k]} onChange={e => setForm(p=>({...p,[k]:e.target.value}))} />
              </Grid>
            ))}
            <Grid item xs={12}><Typography variant="subtitle2" fontWeight={700} color="error">🇲🇳 Монгол мэдээлэл</Typography></Grid>
            {[['mn_name','Нэр *'],['mn_phone','Утасны дугаар *'],['mn_address','Хаяг']].map(([k,l]) => (
              <Grid item xs={12} sm={4} key={k}>
                <TextField fullWidth size="small" label={l} required={l.includes('*')} value={(form as any)[k]} onChange={e => setForm(p=>({...p,[k]:e.target.value}))} />
              </Grid>
            ))}
            ...
            {[['quantity','Тоо ширхэг','number'],['weight','Жин (кг)','number'],['description','Тайлбар','text']].map(([k,l,t]) => (
              <Grid item xs={12} sm={4} key={k}>
                <TextField fullWidth size="small" label={l} type={t} value={(form as any)[k]} onChange={e => setForm(p=>({...p,[k]:e.target.value}))} />
              </Grid>
            ))}
            <Grid item xs={12}><Typography variant="subtitle2" fontWeight={700}>💳 Төлбөр</Typography></Grid>
            {[['paid_in_korea','Солонгосд төлсөн'],['total_fee','Нийт төлбөр'],['remaining_fee','Үлдэгдэл']].map(([k,l]) => (
              <Grid item xs={12} sm={4} key={k}>
                <TextField fullWidth size="small" label={l} type="number" value={(form as any)[k]} onChange={e => setForm(p=>({...p,[k]:e.target.value}))} InputProps={{ endAdornment: '₮' }} />
              </Grid>
            ))}
            <Grid item xs={12}>
